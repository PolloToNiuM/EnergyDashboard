...
import { fetchMeasurements, syncActualGeneration } from "../api/measurements";
import {
...
const selectedProductionTypes = ref(["NUCLEAR", "SOLAR", "WIND_ONSHORE", "WIND_OFFSHORE", "TOTAL"]);

const filteredMeasurements = computed(() =>
  measurements.value.filter((measurement) =>
    selectedProductionTypes.value.includes(measurement.production_type),
  ),
);
...
async function loadMeasurements() {
  ...
  try {
    ...
    if (measurements.value.length === 0) {
      ...
      await syncActualGeneration(selectedDate.value);
      measurements.value = await fetchMeasurements({
        metric: "actual_generation_per_production_type",
        start_date: startDate,
        end_date: endDate,
  ...
  <main class="dashboard-view">
    <header class="dashboard-header">
      <div>
        ...
        <h1>Production electrique RTE</h1>
      </div>
    ...
    <section class="chart-section">
      <div class="section-heading">
        <div>
          <p class="eyebrow">Actual generation</p>
          <h2>Production par filiere</h2>
        </div>
      ...
      <div class="energy-filters" aria-label="Filtres production">
        <label
          v-for="productionType in productionTypeOptions"
          :key="productionType"
          class="energy-filter"
        >
          <input
            v-model="selectedProductionTypes"
            type="checkbox"
